"""compare_compositions.py — side-by-side validation of ONE policy across several team
compositions, in a single figure (top row = agent paths, bottom row = frag-vs-step).
Read-only. Quick visual check that the SAME policy adapts per composition.

Edit the CONFIGS / EXPECT lists below to choose which (n_strikers, n_jammers) to show
and the expected behaviour label. Default set is the κ=2 case. For per-config numeric
metrics (frag, jammer imbalance, balance) use analyze_formations.py instead.

Example (repo root, project venv):
  .venv\\Scripts\\python.exe 0_TA_HF_FOFE-MAPPO\\compare_compositions.py \\
      --checkpoint runs\\composition_agnostic_k2.pt --seed 1002 --out runs\\compare.png
"""
