"""analyze_formations.py — roll out a trained checkpoint at a FIXED team composition
and quantify the emergent formation behaviour. Read-only (never modifies a checkpoint).

For `--seeds` domain-randomised layouts it prints, and renders a per-seed figure of
(agent paths + frag-vs-step):
  • episode-mean frag         coalition fragmentation (0 = one blob, 0.667 = two pairs,
                              0.60 = two 3-agent formations) — the env's own KPI.
  • striker-striker distance  how far the strikers spread (vs coalition_radius).
  • frac time on DIFFERENT tgts  do the strikers split across targets? (1.0 = always).
  • jammers/striker IMBALANCE max-min nearest-assigned jammer count (0 = even e.g. 2-2,
                              2 = a 1-3 split). FRAG IS BLIND TO THIS — use it for κ>1.
  • frac time BALANCED split   fraction of frames with an even jammer split (want ~1.0).

The same trained policy can be evaluated at any (n_strikers, n_jammers) because the
legacy per-role actor obs is fixed-slot; --n_strikers/--n_jammers override the checkpoint.

Examples (repo root, project venv):
  .venv\\Scripts\\python.exe 0_TA_HF_FOFE-MAPPO\\analyze_formations.py \\
      --checkpoint runs\\composition_agnostic_k2.pt --n_strikers 2 --n_jammers 4 --seeds 6 \\
      --out runs\\fig_2s4j.png
See also compare_compositions.py for a side-by-side across several compositions.
"""
