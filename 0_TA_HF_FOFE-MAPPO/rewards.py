...
class RewardConfig:
    ...
    # ─── FORMATION COHESION  (DEPRECATED — superseded by JAMMER ESCORT below) ──
    # Legacy cross-role proximity term. Kept only for backward compatibility
    # with saved configs/checkpoints; defaults are 0 so the term is inert.
    # The jammer-escort coverage reward below replaces its behaviour with a
    # team-coverage formulation that does not reward jammers for crowding an
    # already-escorted striker. Set a scale > 0 to re-enable the old term.
    ...
    escort_over_scale:    float = 0.010   #OLD: 0.015 w_over — [×0.5 rescale of 0.03] jammer penalty per unit of OVER-coverage Σ_s(c_s−κ)₊. Makes the BALANCED split (each striker exactly κ jammers) the unique optimum, so a κ-th+1 jammer piling on one striker is pushed to an under-served one. Without it the escort only punishes under-coverage → 1-3 splits (not 2-2) when κ>1. Set 0 to disable (recovers under-only behaviour).
    escort_commit_temp:   float = 0.01   #NOT USED τ — softmax temperature for jammer→striker commitment (0=hard nearest; ≳striker spread=shared). Set very large to recover pure additive coverage.
    # ─── ESCORT KERNEL SHAPE ──────────────────────────────────────────────────
    ...
    target_cover_kernel_length: float = 0.1    # ℓ_t — target coverage kernel length (map units); range over which the unmet-coverage gradient pulls a striker onto a target
    ...
    target_cover_scale:         float = 0.01   # w_st — NEGATIVE penalty per unit of unmet target coverage (max |penalty| = w_st·Σκ_t); 0 at the desired one-striker-per-target state
